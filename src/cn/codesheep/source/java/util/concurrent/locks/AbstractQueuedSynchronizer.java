...
public abstract class AbstractQueuedSynchronizer
    ...
    private Node enq(final Node node) {
        for (;;) {
            ...
            if (t == null) { // Must initialize
                /**
                 * 设置头节点，初始情况下，头节点是一个空节点
                 */
                if (compareAndSetHead(new Node()))
                    tail = head;
            } else {
                /*＊
                 * 将节点插入队列尾部。这里是先将新节点的前驱设为尾节点，之后再尝试将新节点设为尾节
                 ...
                 *    }
                 *
                 * 但如果这样做，会导致一个问题，即短时内，队列结构会遭到破坏。考虑这种情况，
    ...
    private void cancelAcquire(Node node) {
         ...
         * 再利用 CAS 将prev 的next引用置空，断开与后继节点的联系，完成清理工作
         */
        if (node == tail && compareAndSetTail(node, pred)) {
            /**
             * 执行到这里，表明pred节点被成功设为了尾节点，这里通过CAS将pred节点的后继节点设为null。
             * 注意这里的CAS即使失败了，也没关系。失败了，表明pred的后继节点更新了，pred此时已经是尾节点了，
             * 若后继节点被更新，则是有新节点入队了。这种情况下，CAS会失败，但失败不会影响同步队列的结构。
             */
            ...
            if (pred != head &&
                ...
                /**
                 * 这里使用CAS设置pred的next，表明多个线程同时在取消，这里存在竞争。
                 * 不过此处没针对 compareAndSetNext 方法失败后做一些处理，表明即使失败了也
                 * 没关系。实际上，多个线程同时设置pred的next引用时，只要有一个能设置成功即可。
                 */
     ...
     *    —————————————————————————————————————————————————————————————————
     *    | SIGNAL       阻塞                                           |
     *    | CANCELLED    取消 | 向前遍历, 移除前面所有为该状态的节点               |
     ...
     */
    /**
     *  同步队列中的线程在此方法中以循环尝试获取同步，在有限次的尝试后，
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
                ...
                /**
                 * 前驱节点如果是头节点，表明前驱节点已经获取了同步状态。前驱节点释放同步状态后，
     ...
     */
    /**
     * 独占式获取同步状态，该方法将会调用子类复写的 tryAcquire 方法获取同步状态，
     ＊ - 获取成功：直接返回
